<p align="right"><a href="./README.en.md">English</a></p>

# 📐 Algorithms

> Kho tài liệu **thuật toán và cấu trúc dữ liệu** từ cơ bản đến nâng cao: sách tham khảo quốc tế và giáo trình tiếng Việt.

---

## 📂 Nội dung

| Thư mục | Mô tả |
|---------|-------|
| [Reference](./Reference/) | Sách tham khảo quốc tế (CLRS, Skiena, Knuth TAOCP…) |
| [Textbooks](./Textbooks/) | Giáo trình & slide tiếng Việt |
| [Exercises](./Exercises/) | Bài tập, tài liệu thực hành (`.doc`) |

---

## 📘 Sách tham khảo nổi bật

| Tên tài liệu | Tải về |
|--------------|--------|
| **Introduction to Algorithms – CLRS (3rd Ed.)** | [📥 PDF](./Reference/introduction-to-algorithms-clrs-3rd-edition-sep-2009.pdf) |
| **Algorithm Design – Kleinberg & Tardos** | [📥 PDF](./Reference/algorithm-design.pdf) |
| **The Algorithm Design Manual – Skiena** | [📥 PDF](./Reference/skiena-thealgorithmdesignmanual.pdf) |
| **TAOCP – Knuth (Vol. 1–4)** | [📂 Thư mục TAOC](./Reference/TAOC/) |

---

## 📌 Liên quan

- **Nhập môn / KTLT**: [Intro-to-Programming](../Intro-to-Programming/)
- **Luyện thi đấu**: [Competitive](../Competitive/)
- **Lý thuyết đồ thị**: [Graph-Theory](../Graph-Theory/)

---

<p align="center">
  Góp tài liệu? 👉 <a href="https://github.com/TienNHM/ebooks/pulls">Gửi Pull Request</a>
</p>
