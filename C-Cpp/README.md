# ⚙️ C & C++

> Bộ sưu tập ebook **C** và **C++**: sách tham khảo quốc tế, giáo trình đại học và bài tập thực hành.

---

## 📂 Nội dung

| Thư mục | Mô tả |
|---------|-------|
| [Reference](./Reference/) | Sách tham khảo quốc tế (K&R, King, Head First, C++…) |
| [Textbooks](./Textbooks/) | Giáo trình & slide tiếng Việt |
| [Exercises](./Exercises/) | Bài tập, đề thi, tài liệu HTML |

---

## 📘 Sách nổi bật

| Tên tài liệu | Tải về |
|--------------|--------|
| **The C Programming Language – K&R** | [📥 PDF](./Reference/C/The%20C%20Programming%20Language.pdf) |
| **C Programming: A Modern Approach – King** | [📦 RAR](./Reference/C/%5BK.%20N.%20King%5D%20C%20Programming%20A%20Modern%20Approach%20(2nd%20Edition).rar) |
| **Head First C (3rd Ed.)** | [📥 PDF](./Reference/C/head-first-c-3rd-edition.pdf) |
| **A Complete Guide to Programming in C++** | [📥 CHM](./Reference/Cpp/A%20Complete%20Guide%20to%20Programming%20in%20C++.chm) |

---

## 📌 Liên quan

- **OOP / C++**: [OOP](../OOP/)
- **Kỹ thuật lập trình**: [Intro-to-Programming](../Intro-to-Programming/)
- **Luyện thi**: [Competitive](../Competitive/)

---

<p align="center">
  Góp tài liệu? 👉 <a href="https://github.com/TienNHM/ebooks/pulls">Gửi Pull Request</a>
</p>
